    ...
    <div className="min-h-screen relative p-4 md:p-12">
      ...
      <main className="max-w-6xl mx-auto relative z-10 space-y-12 pb-20 pt-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
          <div>
            <h1 className="text-4xl md:text-5xl font-bold text-slate-100 mb-4">Arwan&apos;s Ecosystem</h1>
          ...
          <div className="flex gap-4">
            <Link
              href="/"
              className="px-6 py-2.5 bg-white/10 hover:bg-white/5 border border-white/10 text-slate-100 font-medium rounded-full shadow-sm transition-all"
            >
              Resume
            </Link>
            <Link
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-slate-100">Hosted Applications</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {hostedApps.map(app => (
              <GlassPanel key={app.id} className="p-6 flex flex-col group hover:-translate-y-1 transition-transform">
                <h3 className="text-xl font-bold text-slate-100 mb-2 group-hover:text-[#7ec8e3] transition-colors">{app.name}</h3>
                <p className="text-slate-300 mb-6 flex-grow">{app.desc}</p>
                <a href={app.url} className="text-sm font-semibold text-slate-100 flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-slate-100">Free Tools</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {freeTools.map(tool => (
              <GlassPanel key={tool.id} className="p-6 flex flex-col relative overflow-hidden group">
                ...
                <p className="text-slate-300 mb-6 flex-grow">{tool.desc}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <section>
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-gray-200 text-slate-300 rounded-lg"><Code2 size={24} /></div>
              <h2 className="text-2xl font-bold text-slate-100">Open Source Showcase</h2>
            </div>
          ...
          <section>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-slate-100">Platform Stats</h2>
            </div>
            <GlassPanel className="p-8 h-[calc(100%-3rem)] flex flex-col justify-center gap-8">
              <div className="flex justify-between items-center border-b border-white/40 pb-4">
                <span className="text-slate-300 font-medium">Total Projects Deployed</span>
                <span className="text-3xl font-bold text-slate-100">24</span>
              </div>
              <div className="flex justify-between items-center border-b border-white/40 pb-4">
                <span className="text-slate-300 font-medium">Lines of Code</span>
                <span className="text-3xl font-bold text-slate-100">~150k</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-300 font-medium">Coffee Cups Consumed</span>
