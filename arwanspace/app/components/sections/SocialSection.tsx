...
export default function SocialSection({ links }: { links: Record<string, string> }) {
  ...
  return (
    <section id="social" className="reveal scroll-mt-24 pb-20">
      <h2 className="text-3xl font-bold text-gray-800 mb-8 pl-4 border-l-4 border-gray-400">Connect With Me</h2>
        ...
        {Object.entries(links).map(([key, url]) => {
          ...
          return (
            <a key={key} href={url} target="_blank" rel="noreferrer" className="group">
              <GlassPanel className="w-16 h-16 rounded-full flex items-center justify-center text-gray-600 group-hover:text-[#7ec8e3] group-hover:scale-110 transition-all duration-300">
                {getIcon(key)}
              </GlassPanel>
            </a>
          );
